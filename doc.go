...
Although supported by this package, it is largely unused by applications and is usually omitted.

The canonical and advanced forms differ in their definitions of `simple-string'.
...
decimal value `nbytes' (with no leading zeroes). There is no white space. It
...
form, either using hexadecimal or base 64 encodings, or using quoted strings
with escape sequences similar to those of Go, Limbo or C.
Unquoted text is called a `token', and is restricted by the standard to a
...
as must non-ASCII characters in UTF-8 encoding.
This implementation allows tokens to start with a digit on input, but quotes them on output.
The advanced transport syntax is defined by the complete set of rules below:
	...
	base-64	::=	nbytes? '|' ( base-64-char | whitespace )* '|'
...
characters; note that it can appear only at the places shown. The bytes in a
`quoted-string-body' are interpreted using quoting rules similar to Go, Limbo or
C. That is, the bytes are enclosed in quotes, and may contain the escape
sequences for the following characters: backspace ( "\b" ), form-feed ( "\f" ),
newline ( "\n" ), carriage-return ( "\r" ), tab ( "\t" ), and vertical tab ("\v" ),
octal escape "\ooo" (all three digits must be given), hexadecimal escape
"\xhh" (both digits must be given), "\\" for backslash, "\'" for single quote,
and `\"' to include a quote in a string. Note that a quoted string, like `base-64', can have an
optional `nbytes', but it gives the length of the resulting byte string after
interpreting character escapes, not number of following bytes.

Both canonical and advanced forms can contain binary data verbatim. Sometimes
that is troublesome for storage or transport. At the lexical level any sexpr
...
expressed in either canonical or advanced form. The S-expression parser will
replace the sequence by its decoded value, and resume parsing at the start of
...
It is a list of two elements: the string "hello world!" and another list also
with two elements, the string "inner" and an empty string (zero bytes). All the
